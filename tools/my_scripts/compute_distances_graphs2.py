...
def main():
    ...
    with open(caminho_arquivo_saida, modo_abertura) as file_saida:
        if modo_abertura == 'w':
            file_saida.write("INSTANCIA,GRAUS,DIST_F\n")
            file_saida.write(f"{nome_arquivo_base}-1.txt {' '.join(map(str, sorted(graus_base, reverse=True)))} 0.00\n")
        ...
        while True:
            ...
            distancia_formatada = "{:.4f}".format(distancia)
