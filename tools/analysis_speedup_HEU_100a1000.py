...
for item in lista_diretorio:
    isDirectory = os.path.isdir(root_dir + "/"+ item)
    if isDirectory is False or not item.endswith('-01'):
        continue
    aux = root_dir + '/' + item
    lista_diretorio1 = os.listdir(aux)
    lista_diretorio1.sort()
    reg = re.compile(r'^grafos')
    ...
    for directory in lista_diretorio2:
        ...
        for filename in lista_files:
            if not filename.endswith('.out.txt'):
                continue
            aux3 = aux2 + '/' + filename
