...
running_d = ['0_ct-bf-par-v1-08072023-01',
             '0_ct-bf-edges-v1-09072023-01',
             '0_ct-bf-edges-08072023-01',
             '0_ct-bf-adjacency-07072023-01',
             '0_ct-bf-induced-CI_1-06072023-01',
             '0_ct-bf-induced-CI_2-07072023-01']
                ...
                while True:
                    ...
                    if "INSTANCE" in textfile:
                        ...
                        if "CI_1" in folder:
                            ...
                            data["solution"] = "CYCLEe_1"
                        elif "CI_2" in folder:
                            #data["solution"] = value + "_2"
                            data["solution"] = "CYCLEe_2"
                        elif "ADJACENCY_LIST" in value:
                            #data["solution"] = value + "_2"
                            data["solution"] = "ADJACENCIES"
                        elif "EDGE_PARALELISM" in value:
                            #data["solution"] = value + "_2"
                            data["solution"] = "EDGES"
                        elif "EDGE_PARALLELISM" in value:
                            #data["solution"] = value + "_2"
                            data["solution"] = "ADJv1"
                        # elif "par-v1" in folder and "EDGE_PARALLELISM" in value:
                        #     data["solution"] = "ADJv1"
                        else:
                            data["solution"] = value
                    elif "NUM_VERTICES" in textfile:
...
for folder in main_dictionary:
    for instance in main_dictionary[folder]:
        for nthread in main_dictionary[folder][instance]:
            ...
            #print(main_dictionary[folder][instance][nthread]['speedup'])
            #print(main_dictionary[folder][instance][thread]['solution'], main_dictionary[folder][instance][thread]['vertex'], nthread, main_dictionary[folder][instance][thread]['mean'], "Speedup", baseline_dict[base][instance]['1']['mean'] / main_dictionary[folder][instance][thread]['mean'])
...
for folder in main_dictionary:
    ...
    for instance in main_dictionary[folder]:
        ...
        for thread in x_axis:
            ...
            running[instance][solution].append(speedup)
...
for instance in running:
    ...
    for solution in running[instance]:

        #for solution in running[folder][instance]:
        parada = running[instance][solution]
        #ax1.plot(x_axis, parada, label=string)
        ax1.plot(x_axis, parada, label=solution)


    ax1.set_ylabel("speedup")
    ax1.set_xlabel("threads")
    ax1.title.set_text(instance)
    ax1.legend(loc = 'lower right', fontsize = '5', title = 'Legend', title_fontsize = '3')
    #ax1.set_yscale('symlog')
    #plt.show()
    plt.grid(True)
    plt.savefig(images_dir + '/speedup-' + instance + "_" + baseline_d[0] + '.png', dpi=300, format='png', bbox_inches='tight')
    plt.clf()
    plt.close()
