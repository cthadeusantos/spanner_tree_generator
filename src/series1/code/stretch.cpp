...
void Stretch::find_index(Graph& g)
{
    ...
    int ult_colocado[num_vertex];
    int v = 0;
    int u;
    unsigned long long int total_local_trees = 0;
    int grt;
    //index = INF_VALUE;
    //index = (int)INFINITY;
    ...
    while( v >= 0 and !(global_parameters.global_abort_for_timeout)){
        if( prox_vizinho[v] == g.getDegree(v) ){
            ...
            tree.removeEdge(v, ult_colocado[v]);
            ult_colocado[v] = -1;

        }else{
            //u = g.adjList(v)[prox_vizinho[v]];
            u = g.getAdjacentVertices(v)[prox_vizinho[v]];
            ++prox_vizinho[v];
            if( not tree.has_edge(v, u) ) {
                tree.addEdge(v, u);
                ult_colocado[v] = u;
                ...
                //if(not OpBasic::is_cyclic(tree)){
                    if(tree.numEdges() == tree.numVertices() - 1){
                        int f=1;
                        if (!global_parameters.global_noindex) {
    ...
    //this->total_arv = total_local_trees;
    g.sum_trees(total_local_trees);
...
//     int indice[n-1];
//     int grt;
//     std::vector<int> edges = g.edgeList();
...
//             --j;
//             tree.removeEdge(edges[indice[j]], edges[indice[j]+1]);
//             indice[j] += 2;
//         }
//         else {
//             tree.addEdge(edges[indice[j]], edges[indice[j]+1]);
